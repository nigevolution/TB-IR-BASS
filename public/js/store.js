document.addEventListener("DOMContentLoaded", () => {
  ...
  const produtos = [
    ...
    /* ===== LANÇAMENTOS COM ÁUDIO + CRONÔMETRO ===== */
    {
      ...
      html += `
        <div class="audio-wrap">
          ...
          <button class="play-btn"
            onclick="this.previousElementSibling.play()">▶ Preview 30s</button>
        </div>
      `;
    }
  ...
  document.querySelectorAll(".countdown").forEach(el=>{
    ...
    const timer = setInterval(()=>{
      ...
      if(diff <= 0){
        ...
        el.innerHTML = `
          ${price ? `<div class="price">R$ ${Number(price).toFixed(2).replace(".",",")}</div>` : ``}
          ${link ? `<button onclick="window.open('${link}')">Comprar agora</button>` : ``}
        `;
        clearInterval(timer);
        return;
      }
