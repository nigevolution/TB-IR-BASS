const produtos = [
  ...
  {nome:"Sadowsky NYC IR",preco:59,desc:"Flagship nova-iorquino com graves profundos.",release:"2026-01-13T19:00:00"},
  {nome:"Mayones Jabba 5 IR",desc:"Flagship europeu.",release:"2026-01-20T19:00:00"},
  {nome:"MTD 535-24 IR",desc:"Boutique luthier.",status:"LANÇAMENTO EM BREVE"},
...
produtos.forEach(p=>{
  ...
  if(btn){
    btn.onclick=()=>{
      if(tocando && tocando!==audio){tocando.pause();tocando.currentTime=0;}
      tocando=audio;
      audio.currentTime=0;
      audio.play();
      btn.classList.add("playing");
      audio.ontimeupdate=()=>{ if(audio.currentTime>=30){audio.pause();btn.classList.remove("playing");}}
...
setInterval(()=>{
  document.querySelectorAll(".countdown").forEach(el=>{
    const d=new Date(el.dataset.date).getTime()-Date.now();
    if(d<=0){el.innerHTML="🔥 DISPONÍVEL";return;}
